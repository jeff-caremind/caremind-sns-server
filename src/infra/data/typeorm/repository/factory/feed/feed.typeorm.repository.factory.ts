...
import { ITypeormRepositoryFactory } from '../typeorm.repository.factory.interface.js';

export class FeedTypeormRepositoryFactory
  implements ITypeormRepositoryFactory<FeedVo>
{
  getRepository = (dataSource: DataSource): IFeedTypeormRepository => {
    const repository = dataSource.getRepository(FeedVo);
    const feedTypeormRepository = this.extendRepository(repository);
